...
<form class="uk-form-stacked uk-padding-small" action="" method="post" autocomplete="off" enctype="multipart/form-data">
    <div class="uk-margin">
        ...
        <div class="uk-form-controls">
            <textarea class="uk-textarea uk-form-small <?= form_error('body') ? 'uk-form-danger' : '' ?>" id="body" rows="10" name="body" placeholder="<?= ucwords($this->lang->line('body')); ?>..."><?= isset($about) ? $about->body : set_value('body'); ?></textarea>
        </div>
        <?= form_error('body', '<p class="uk-text-danger uk-text-small uk-margin-small-top">', '</p>'); ?>
    </div>
    <div class="uk-margin">
        <label class="uk-form-label" for="short_body"><?= ucwords($this->lang->line('short_body')); ?></label>
        <div class="uk-form-controls">
            <textarea class="uk-textarea uk-form-small <?= form_error('short_body') ? 'uk-form-danger' : '' ?>" id="short_body" rows="5" name="short_body" placeholder="<?= ucwords($this->lang->line('short_body')); ?>..."><?= isset($about) ? $about->short_body : set_value('short_body'); ?></textarea>
        </div>
    ...
    <div class="uk-margin">
        <div class="uk-flex uk-flex-center">
            <div>
                <button class="uk-button uk-button-primary" type="submit"><?= isset($about) ? ucwords($this->lang->line('update')) : ucwords($this->lang->line('save')); ?></button>
            </div>
