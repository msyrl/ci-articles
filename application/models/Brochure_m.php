...
class Brochure_m extends MY_Model
{
    protected $_table_name = 'brochures';
    ...
    public function get_with_paginate($page = 1, $per_page = 10, $is_publish = FALSE)
    {
        ...
        $data = $this->db
            ->select("$this->_table_name.id as id, $this->_table_name.title as title, $this->_table_name.image as image, $this->_table_name.attachment as attachment, users.username as updated_by, $this->_table_name.created_at as created_at, $this->_table_name.is_publish as is_publish")
            ->join('users', 'brochures.updated_by = users.id');
        if ($is_publish) {
